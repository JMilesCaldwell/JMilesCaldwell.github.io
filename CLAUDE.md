...
*Body: dashboard feed-health diagnostic. Probes every RSS URL the dashboard uses (news Local / National / World + Discover panel sources), renders a status table, dumps `discover.json` array sizes, and exposes a "Copy results JSON" button so Joe can hand the full blob back. Run by visiting `/sandbox.html` after the dashboard is deployed. As of 2026-09-02 the National rows carry candidate URL *variants* (Investico A-D, Kanttekening A-B) rather than one URL each — the point of that run is to find which path works, then prune the losers from both this list and `dashboard.html`. Memory log: many entries — bootstrap, security, aesthetic conventions, Zwolle context, news editorial intent, weather + map decisions, Discover panel rationale.*
